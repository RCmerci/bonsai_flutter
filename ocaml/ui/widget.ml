...
let kind_tag_to_string = function
  ...
  | K_material_filled_button -> "Material_filled_button"
  | K_material_filled_tonal_button -> "Material_filled_tonal_button"
  | K_material_outlined_button -> "Material_outlined_button"
  ...
  | K_material_simple_dialog -> "Material_simple_dialog"
...
let node_kind_tag (type k) (n : k node) : kind_tag =
  ...
  | Material_filled_button _ -> K_material_filled_button
  | Material_filled_tonal_button _ -> K_material_filled_tonal_button
  | Material_outlined_button _ -> K_material_outlined_button
  ...
  | Material_simple_dialog _ -> K_material_simple_dialog
...
let node_equal (type k1 k2) (a : k1 node) (b : k2 node) : bool =
  ...
  | Material_scaffold x, Material_scaffold y ->
    ...
    && Bool.equal x.has_floating_action_button y.has_floating_action_button
  ...
  | Material_filled_button x, Material_filled_button y ->
    ...
    && Bool.equal x.enabled y.enabled
    && Bool.equal x.autofocus y.autofocus
  | Material_filled_tonal_button x, Material_filled_tonal_button y ->
    x.variant = y.variant
    && Bool.equal x.enabled y.enabled
    && Bool.equal x.autofocus y.autofocus
  | Material_outlined_button x, Material_outlined_button y ->
    x.variant = y.variant
    && Bool.equal x.enabled y.enabled
    && Bool.equal x.autofocus y.autofocus
  | Material_floating_action_button x, Material_floating_action_button y ->
    x.variant = y.variant
    && Bool.equal x.enabled y.enabled
    && Bool.equal x.autofocus y.autofocus
  ...
  | Material_radio_group x, Material_radio_group y ->
    ...
    && List.equal
         (fun (left : material_radio_option) (right : material_radio_option) ->
  ...
  | Material_slider x, Material_slider y ->
    ...
    && Float.equal x.max y.max
    && Option.equal Int.equal x.divisions y.divisions
    && Option.equal String.equal x.label y.label
    && Bool.equal x.enabled y.enabled
  ...
  | Material_range_slider x, Material_range_slider y ->
    ...
    && Float.equal x.max y.max
    && Option.equal Int.equal x.divisions y.divisions
    && Option.equal String.equal x.label_start y.label_start
  ...
  | Material_data_table x, Material_data_table y ->
    ...
    && Bool.equal x.has_on_select_all y.has_on_select_all
    && Bool.equal x.has_on_cell_activate y.has_on_cell_activate
  | Material_stepper x, Material_stepper y ->
    x.orientation = y.orientation
    && Int64.equal x.current_step_id y.current_step_id
    && List.equal ( = ) x.steps y.steps
  | Material_expansion_panel_list x, Material_expansion_panel_list y ->
    x.policy = y.policy
    && List.equal Int64.equal x.expanded_ids y.expanded_ids
    && List.equal ( = ) x.panels y.panels
  | Material_simple_dialog x, Material_simple_dialog y ->
    Bool.equal x.has_title y.has_title && List.equal ( = ) x.options y.options
  | Material_fullscreen_dialog, Material_fullscreen_dialog -> true
  ...
  | Material_divider x, Material_divider y ->
    ...
    && Float.equal x.indent y.indent
    && Float.equal x.end_indent y.end_indent
...
let theme ?key ~data child =
  create_typed
    ~key
    ~node:(Theme (Theme.Private.view_data data))
    ~event_bindings:[||]
    ~children:(plain_children [ child ])
;;
...
let material_scaffold_widget
  ...
  create_typed
    ...
    ~node:
      (Material_scaffold
         { has_app_bar = Option.is_some app_bar
         ; has_floating_action_button = Option.is_some floating_action_button
         ; floating_action_button_location
         ; has_bottom_navigation_bar = Option.is_some bottom_navigation_bar
         ; has_bottom_sheet = Option.is_some bottom_sheet
         })
    ~event_bindings:[||]
    ~children:
      (plain_children
         (Option.to_list app_bar
          @ Option.to_list floating_action_button
          @ Option.to_list bottom_navigation_bar
          @ Option.to_list bottom_sheet
          @ [ body ]))
;;
...
let material_button
  ...
  | Filled ->
    create_typed
      ~key
      ~node:(Material_filled_button { variant; enabled; autofocus })
      ~event_bindings
      ~children
  | Filled_tonal ->
    create_typed
      ~key
      ~node:(Material_filled_tonal_button { variant; enabled; autofocus })
      ~event_bindings
      ~children
  | Outlined ->
    create_typed
      ~key
      ~node:(Material_outlined_button { variant; enabled; autofocus })
      ~event_bindings
      ~children
  | Elevated ->
    create_typed
...
let material_floating_action_button
      ?key
      ?(enabled = true)
      ?(autofocus = false)
      ~variant
...
let material_radio_group ?key ~selected_id ~options ~children ~on_select () =
  create_typed
    ~key
    ~node:(Material_radio_group { selected_id; options })
    ~event_bindings:[| { tag = Event.Tag.Radio_selected; handler = on_select } |]
    ~children:(plain_children children)
;;
...
let material_slider
  ...
  let event_bindings =
    Array.of_list
      (Option.to_list
         (Option.map
            (fun handler -> { tag = Event.Tag.Slider_changed; handler })
            on_change)
       @ [ { tag = Event.Tag.Slider_change_end; handler = on_change_end } ])
  in
  create_typed
    ~key
    ~node:
...
let material_range_slider
  ...
  let event_bindings =
    Array.of_list
      (Option.to_list
         (Option.map
            (fun handler -> { tag = Event.Tag.Range_slider_changed; handler })
            on_change)
       @ [ { tag = Event.Tag.Range_slider_change_end; handler = on_change_end } ])
  in
  create_typed
    ~key
    ~node:
  ...
  let event_bindings =
    ...
    @ Option.to_list
        (Option.map
           ...
           on_limit_reached)
    @ Option.to_list
        (Option.map (fun handler -> { tag = Event.Tag.Press; handler }) on_tap)
  in
  create_typed
    ~key
    ~node:
      (Material_search_bar
         ...
         ; max_utf8_bytes
         ; has_leading = Option.is_some leading
         ; trailing_count = List.length trailing
         ; hint_text
         ; has_on_tap = Option.is_some on_tap
         })
    ~event_bindings:(Array.of_list event_bindings)
  ...
  create_typed
    ...
    ~node:
      (Material_data_table
         ...
         ; has_on_select_all = Option.is_some on_select_all
         ; has_on_cell_activate = Option.is_some on_cell_activate
         })
    ~event_bindings:
      (Array.of_list
         (List.filter_map
            ...
            ; binding Event.Tag.Table_select_all on_select_all
            ; binding Event.Tag.Table_cell_activated on_cell_activate
            ]))
    ~children:(plain_children children)
;;

let material_stepper
      ?key
      ~orientation
      ~current_step_id
      ~steps
      ...
      ()
  =
  let optional tag = function
    | None -> []
    | Some handler -> [ { tag; handler } ]
  in
  create_typed
    ~key
    ~node:(Material_stepper { orientation; current_step_id; steps })
    ~event_bindings:
      (Array.of_list
         (optional Event.Tag.Step_selected on_step_selected
          @ optional Event.Tag.Step_continue on_continue
          @ optional Event.Tag.Step_cancel on_cancel))
    ~children:(plain_children children)
;;

let material_expansion_panel_list
      ?key
      ~policy
      ~expanded_ids
      ~panels
  ...
  create_typed
    ~key
    ~node:(Material_expansion_panel_list { policy; expanded_ids; panels })
    ~event_bindings:
      [| { tag = Event.Tag.Expansion_changed; handler = on_expansion_changed } |]
    ~children:(plain_children children)
;;

let material_simple_dialog ?key ?title ~options ~on_select ~children () =
  create_typed
    ~key
    ~node:(Material_simple_dialog { has_title = Option.is_some title; options })
    ~event_bindings:[| { tag = Event.Tag.Dialog_option_selected; handler = on_select } |]
    ~children:(plain_children (Option.to_list title @ children))
;;

let material_fullscreen_dialog ?key child =
  create_typed
    ~key
    ~node:Material_fullscreen_dialog
    ~event_bindings:[||]
    ~children:(plain_children [ child ])
;;
...
let material_divider
      ?key
      ?(orientation = Layout.Axis.Horizontal)
      ?(thickness = 1.)
      ?(spacing = 16.)
      ?(indent = 0.)
      ?(end_indent = 0.)
  ...
  create_typed
    ...
    ~node:
      (Material_divider
         { orientation
         ; thickness = finite_nonnegative "thickness" thickness
         ; spacing = finite_nonnegative "spacing" spacing
         ; indent = finite_nonnegative "indent" indent
         ; end_indent = finite_nonnegative "end_indent" end_indent
         })
    ~event_bindings:[||]
    ~children:[||]
;;

let material_card ?key ?(variant = Elevated_card) ?(elevation = 1.) child =
  create_typed
    ~key
    ~node:
      (Material_card { variant; elevation = finite_nonnegative "elevation" elevation })
    ~event_bindings:[||]
    ~children:(plain_children [ child ])
;;
...
let material_scaffold
  ...
  material_scaffold_widget
    ...
    ?bottom_navigation_bar
    ?bottom_sheet
    ~body:(Body.Private.to_widget body)
    ()
;;
...
module Private = struct
  ...
  type nonrec material_floating_action_button_location =
        material_floating_action_button_location =
    | Start_float
    | Center_float
    | End_float
    | Start_docked
  ...
  type nonrec material_floating_action_button_variant =
        material_floating_action_button_variant =
    | Small
    | Standard
    | Large
    | Extended
  ...
  type nonrec material_chip_presentation = material_chip_presentation =
    | Flat_chip
    | Elevated_chip

  type nonrec material_card_variant = material_card_variant =
    | Elevated_card
    | Filled_card
    | Outlined_card
  ...
  type nonrec material_data_table_cell = material_data_table_cell =
    { placeholder : bool
    ; show_edit_icon : bool
    ; activatable : bool
    }

  type nonrec material_data_table_row = material_data_table_row =
    { row_id : int64
    ; selected : bool
    ; selection_enabled : bool
    ; cells : material_data_table_cell list
  ...
  type nonrec material_step_state = material_step_state =
    | Step_indexed
    | Step_editing
    | Step_complete
    | Step_disabled
  ...
  type nonrec material_step = material_step =
    { step_id : int64
    ; active : bool
    ; state : material_step_state
    ; has_subtitle : bool
  ...
  type nonrec material_expansion_panel_policy = material_expansion_panel_policy =
    | Multiple_panels
    | Single_panel

  type nonrec material_expansion_panel = material_expansion_panel =
    { panel_id : int64
    ; enabled : bool
    ; can_tap_on_header : bool
    }

  type nonrec material_simple_dialog_option = material_simple_dialog_option =
    { option_id : int64
    ; enabled : bool
    }
