...
let validate_unique_keys root =
  let rec validate widget =
    ...
    let keys = Key_table.create (Array.length view.children) in
    let rec check_children index =
      ...
      then Ok ()
        ...
        | Some key when Key_table.mem keys key ->
          Error
            (Runtime_error.Duplicate_key
               { parent_kind = Widget.Private.Kind.to_string view.kind; key })
        | key ->
          Option.iter (fun key -> Key_table.add keys key ()) key;
...
let reconcile t ~base_revision ~target_revision ~old new_widget =
  ...
  then
    ...
    | Ok () ->
      ...
      let rec queue_drop (node : Mounted_tree.Private.node) =
        ...
        drops_reversed := Frame_patch.Operation.Drop_node node.node_id :: !drops_reversed
      in
      let create_bindings event_bindings =
      ...
      let rec mount widget parent_data =
        ...
        let event_bindings, handlers = create_bindings view.event_bindings in
        emit
        ...
        let children =
          Array.map
            (fun (child : Widget.Private.child) -> mount child.widget child.parent_data)
            view.children
        in
      ...
      let handler_entries = ref [] in
      let rec collect_handlers (node : Mounted_tree.Private.node) =
        Array.iteri
          (fun index (binding : Mounted_tree.Mounted_binding.t) ->
             handler_entries
             := { Handler_registry.Frame.node_id = node.node_id
                ; event_tag = binding.event_tag
                ; handler_id = binding.handler_id
                ; handler = node.handlers.(index)
                }
                :: !handler_entries)
          node.event_bindings;
        Array.iter collect_handlers node.children
      in
      collect_handlers root;
      let handler_frame =
        Handler_registry.Frame.Private.create ~revision:target_revision !handler_entries
      in
